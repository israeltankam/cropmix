...
def run_ensemble(
    *,
    ...
    result = {
        ...
        "incidence_sd": (np.std(incidence, axis=0, ddof=1) if len(incidence) > 1 else np.zeros(incidence.shape[1])),
        "mean_vector_prevalence": vectors.mean(axis=0),
        "vector_prevalence_sd": (np.std(vectors, axis=0, ddof=1) if len(vectors) > 1 else np.zeros(vectors.shape[1])),
        ...
        "incidence_susc_sd": (np.nanstd(incidence_susc, axis=0, ddof=1) if len(incidence_susc) > 1 and not np.isnan(incidence_susc).all() else np.zeros(incidence_susc.shape[1])),
        ...
        "incidence_res_sd": (np.nanstd(incidence_res, axis=0, ddof=1) if len(incidence_res) > 1 and not np.isnan(incidence_res).all() else np.zeros(incidence_res.shape[1])),
